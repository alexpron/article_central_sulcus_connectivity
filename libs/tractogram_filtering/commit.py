...
def dipy_to_commit(
    ...
    if not complete_scheme:
        ...
        if gradient is not None:
            ...
            out_metadata[:, 3] = np.sqrt(
                bvals[:] / (out_metadata[:, 4] - (out_metadata[:, 5] / 3.0))
            ) * (0.001 / (267.513 * out_metadata[:, 5]))
...
def commit_filtering(subject):
    ...
    relative_path_dmri = os.path.join(
        "dmri", dwi_acq, dwi_proc, "corrected_dwi_" + sub + ".nii.gz"
    )
    relative_path_scheme = os.path.join(
        "dmri", dwi_acq, dwi_proc, "commit_metadata.txt"
    )
    b0_thr = 50
