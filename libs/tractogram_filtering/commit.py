...
def dipy_to_commit(fbvals, fbvecs, path_out, complete_scheme=False, gradient=None, big_delta=0.0431, small_delta=0.0106,
                   echo_time=0.089):
    """
    ...
    if not complete_scheme:
        ...
        if gradient is not None:
            ...
            out_metadata[:, 3] = np.sqrt(bvals[:] / (out_metadata[:, 4] - (out_metadata[:, 5] / 3.0))) * (
