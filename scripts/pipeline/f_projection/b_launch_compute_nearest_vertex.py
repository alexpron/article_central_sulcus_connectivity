...
if __name__ == '__main__':
    ...
    from configuration.configuration import SUBJ_LIST, SIDES, DIR_PROJECT, DIR_CLUSTER, MESHES, TRACTS_EXTREMITIES, \
        ASSO_TRACT_EXTREMITIES, ASSO_TRACT_NEAREST_VERTEX
        ...
        for side in SIDES.keys():
            for ext in TRACTS_EXTREMITIES:
                if os.path.exists(ASSO_TRACT_EXTREMITIES):
                    ...
                    cmd = os.path.join(DIR_PROJECT, 'libs', 'projection', 'nearest_mesh_vertex_cluster.py') + '  ' + \
                          ASSO_TRACT_EXTREMITIES[(subject, side, ext)] + '  ' + MESHES[
                              (subject, side, 'white')] + '  ' + ASSO_TRACT_NEAREST_VERTEX[(subject, side, ext)]
