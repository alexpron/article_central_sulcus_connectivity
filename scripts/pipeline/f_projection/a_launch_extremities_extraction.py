...
if __name__ == '__main__':
    ...
    from configuration.configuration import SUBJ_LIST, SIDES, DIR_PROJECT, DIR_CLUSTER, ASSOCIATION_TRACTS, ASSO_TRACT_EXTREMITIES
    ...
    for subject in SUBJ_LIST:
        for side in SIDES.keys():
            ...
            cmd = os.path.join(DIR_PROJECT,'scripts','pipeline','d_extract_fibers_extremities','extract_streamlines_extremities_cluster.py') + '  ' + \
                  ASSOCIATION_TRACTS[(subject, side)] + '  ' + ASSO_TRACT_EXTREMITIES[(subject, side, 's')] + '  ' + ASSO_TRACT_EXTREMITIES[(subject, side, 'e')]
