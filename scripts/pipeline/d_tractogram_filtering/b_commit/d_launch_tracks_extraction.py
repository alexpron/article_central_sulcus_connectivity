...
if __name__ == "__main__":
    ...
    for subject in SUBJ_LIST:
        if (
            os.path.exists(TRACTS[(subject, "trk", "raw")])
            and os.path.exists(COMMIT_WEIGHTS[subject])
            and not os.path.exists(TRACTS[(subject, "trk", "filtered")])
        ):
            extract_sub_tractogram(
                TRACTS[(subject, "trk", "raw")],
