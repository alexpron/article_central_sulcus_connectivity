...
if __name__ == "__main__":
    ...
    path_hemispheres_index = os.path.join(
        DIR_OUT, "connectivity", "indexes", "hemispheres_index.npy"
    )
    ...
    for i, subject in enumerate(SUBJ_LIST):
        for j, side in enumerate(SIDES):
            index = len(SIDES) * i + j
            path_U_fibers = os.path.join(
                DIR_OUT, "U_fibers", "arrays", subject + "_" + side + ".npy"
            )
            streamlines = np.load(path_U_fibers)
            clust_stream = streamlines
            subjects = np.concatenate(
                (subjects, int(subject) * np.ones(clust_stream.shape[0]))
            )
            length = get_lengths(clust_stream)
            lengths = np.concatenate((lengths, length))
    df = pd.DataFrame(
        {"Subject": subjects.astype(int), "Hemisphere": hemispheres, "Length": lengths}
    )
    df.to_csv(
        os.path.join(
            DIR_OUT, "connectivity", "coordinates", "raw", "U_fibers_length.csv"
        ),
        index=False,
    )
