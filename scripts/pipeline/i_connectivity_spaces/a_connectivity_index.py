...
if __name__ == '__main__':
    ...
    from configuration.configuration import SUBJ_LIST, SIDES, GYRI, MESHES, ASSO_TRACT_NEAREST_VERTEX, U_FIBERS_MASK, \
        GYRAL_CRESTS, GEO_DISTS, U_FIBERS_INDEXES, HEMI_INDEXES
    ...
    for i, subject in enumerate(SUBJ_LIST):
        for j, side in enumerate(SIDES.keys()):
            if os.path.exists(ASSO_TRACT_NEAREST_VERTEX[(subject, side, 's')]) and os.path.exists(
                    ASSO_TRACT_NEAREST_VERTEX[(subject, side, 'e')]):
                ...
                pre_index, post_index = get_streamline_coord_on_gyri(U_s, U_e, pre, post, dist_to_pre, dist_to_post,
                                                                     mesh)
