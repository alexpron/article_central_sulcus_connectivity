...
if __name__ == "__main__":
    ...
    for i, subject in enumerate(SUBJ_LIST):
        for j, side in enumerate(SIDES.keys()):
            if os.path.exists(
                ASSO_TRACT_NEAREST_VERTEX[(subject, side, "s")]
                ...
                dist_to_pre = np.array(
                    aims.read(GEO_DISTS[(subject, side, GYRI[0])])[0]
                )
                dist_to_post = np.array(
                    aims.read(GEO_DISTS[(subject, side, GYRI[1])])[0]
                )

                pre_index, post_index = get_streamline_coord_on_gyri(
                    U_s, U_e, pre, post, dist_to_pre, dist_to_post, mesh
                )
