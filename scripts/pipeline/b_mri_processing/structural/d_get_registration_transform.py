...
def get_transformation(path_volume,path_reference_volume,path_transfo,path_inverse):
    ...
    transfo_reference_to_mni = aims.AffineTransformation3d(header_ref['transformations'][0])
...
if __name__ =='__main__':
    ...
    for subject in SUBJ_LIST:
        if not os.path.exists(T1_BRAINVISA[subject]):
            print("HCP subject ", subject, "has not been imported yet in HCP Brainvisa database")
            pass
        else:
