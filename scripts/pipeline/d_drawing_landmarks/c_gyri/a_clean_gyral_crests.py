...
from configuration.configuration import SUBJ_LIST, SIDES, GYRI, MESHES, GYRAL_CRESTS, GYRAL_PARAMETRISATIONS
    ...
    for i, subject in enumerate(SUBJ_LIST):
        for j, side in enumerate(SIDES):
            ...
            for k, gyrus in enumerate(GYRI):
                ...
                gyrus_tex = aims.read(GYRAL_CRESTS[(subject, side, gyrus, 'drawn', 'texture')])
                gyrus_a = np.array(gyrus_tex[0])
                #removing additionnal triangles to obtain a real line
                cleaned_gyrus = clean_line(mesh,gyrus_a)
                np.save(GYRAL_CRESTS[(subject, side, gyrus, 'cleaned', 'array')], cleaned_gyrus)
                ...
                aims.write(cleaned_gyrus_tex,GYRAL_CRESTS[(subject, side, gyrus, 'cleaned', 'texture')])
