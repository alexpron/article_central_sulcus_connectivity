...
import pandas as pd
from pipeline.m_visualisation.visualisation import density_plot_group_with_maxima
        ...
        for i, name in enumerate(names):
            ...
            if os.path.exists(path_density):
                ...
                path_fig = os.path.join(DIR_FIG,'connectivity_space','densities','group', side + '_' + name + '_' +
                                    'radius' + '_' + str(factor) + '_with_maxima.tiff' )
                density_plot_group_with_maxima(X,Y, density, pli_passage=pli, path_fig=path_fig, title=title,vmin=0, vmax=density_max)
            else:
                pass
