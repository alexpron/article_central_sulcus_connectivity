...
import pandas as pd
from pipeline.m_visualisation.visualisation import density_plot_subject, density_plot_subject_no_annot, \
    density_plot_subject_no_annot_crests
    ...
    for i, subject in enumerate(SUBJ_LIST):
        for j, side in enumerate(SIDES):
            ...
            for k, name in enumerate(names):
                ...
                path_density = os.path.join(DIR_OUT, 'connectivity', 'densities', 'subjects',
                                            name + '_' + subject + '_' + side
                ...
                if os.path.exists(path_density):
                    ...
                    # title = 'Central Sulcus Subject Connectivity Space: \n' + subject + ' ' + sides[side] + ' ' + \
                    #                                                                       'Hemisphere'
                    path_fig = os.path.join(DIR_FIG, 'connectivity_space', 'densities', 'subjects',
                                            subject + '_' + side + \
                                            '_' +
                                            name + '_' +
                                            'radius' + '_' + str(factor) + '_group_pp_filtered_with_crests.tiff')

                    density_plot_subject_no_annot_crests(X, Y, density, pli_passage_subject=subject_pp,
                                                         pli_passage_group=None,
