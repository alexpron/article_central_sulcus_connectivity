...
if __name__ == "__main__":
    ...
    for l in labels:
        ...
        # print W_var, p_var
        W_var_t, p_var_t = levene(
            L_L_M, L_L_F, L_H_M, L_H_F, R_L_M, R_L_F, R_H_M, R_H_F
        )
        print
        "Levene test, Label:", l, W_var_t, p_var_t
        model = smf.ols(
            "Centroid_Diag_Coord_Iso ~ C(Hemisphere)*C(HandednessQ)*C(Gender)",
            data=new_df,
        ).fit()
        resid = model.resid
