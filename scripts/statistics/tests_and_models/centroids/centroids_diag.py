...
if __name__ == '__main__':
    ...
    for l in labels:
        ...
        L_L = new_df.loc[(new_df['Hemisphere']=='L') & (new_df['HandednessQ']=='Low'),'Centroid_Diag_Coord_Iso'].values
        L_H = new_df.loc[(new_df['Hemisphere']=='L') & (new_df['HandednessQ']=='High'),'Centroid_Diag_Coord_Iso'].values
        R_L = new_df.loc[(new_df['Hemisphere']=='R') & (new_df['HandednessQ']=='Low'),'Centroid_Diag_Coord_Iso'].values
        R_H = new_df.loc[(new_df['Hemisphere']=='R') & (new_df['HandednessQ']=='High'),'Centroid_Diag_Coord_Iso'].values

        L_L_M = new_df.loc[
            (new_df['Hemisphere'] == 'L') & (new_df['HandednessQ'] == 'Low') & (new_df['Gender'] == 'M'), 'Centroid_Diag_Coord_Iso'].values
        L_L_F = new_df.loc[
            (new_df['Hemisphere'] == 'L') & (new_df['HandednessQ'] == 'Low') & (new_df['Gender'] == 'F'), 'Centroid_Diag_Coord_Iso'].values
        L_H_M = new_df.loc[
            (new_df['Hemisphere'] == 'L') & (new_df['HandednessQ'] == 'High') & (new_df['Gender'] == 'M'), 'Centroid_Diag_Coord_Iso'].values
        L_H_F = new_df.loc[
            (new_df['Hemisphere'] == 'L') & (new_df['HandednessQ'] == 'High') & (new_df['Gender'] == 'F'), 'Centroid_Diag_Coord_Iso'].values
        R_L_M = new_df.loc[
            (new_df['Hemisphere'] == 'R') & (new_df['HandednessQ'] == 'Low') & (new_df['Gender'] == 'M'), 'Centroid_Diag_Coord_Iso'].values
        R_L_F = new_df.loc[
            (new_df['Hemisphere'] == 'R') & (new_df['HandednessQ'] == 'Low') & (new_df['Gender'] == 'F'), 'Centroid_Diag_Coord_Iso'].values
        R_H_M = new_df.loc[
            (new_df['Hemisphere'] == 'R') & (new_df['HandednessQ'] == 'High') & (new_df['Gender'] == 'M'), 'Centroid_Diag_Coord_Iso'].values
        R_H_F = new_df.loc[
            (new_df['Hemisphere'] == 'R') & (new_df['HandednessQ'] == 'High') & (new_df['Gender'] == 'F'), 'Centroid_Diag_Coord_Iso'].values

        #W_var , p_var = levene(L_L, L_H, R_L, R_H)
        #print W_var, p_var
        W_var_t, p_var_t = levene(L_L_M, L_L_F, L_H_M, L_H_F,R_L_M, R_L_F, R_H_M, R_H_F)
        print "Levene test, Label:",l,  W_var_t, p_var_t
        model = smf.ols('Centroid_Diag_Coord_Iso ~ C(Hemisphere)*C(HandednessQ)*C(Gender)',data=new_df).fit()
        resid = model.resid
        W_r, p_r = shapiro(resid)
        print "Shapiro Residuals test, Label:", l, W_r,p_r
