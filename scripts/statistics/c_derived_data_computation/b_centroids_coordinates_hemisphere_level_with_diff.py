...
if __name__ == "__main__":
    ...
    df_final["Centroid_PP_Diag_Diff_Iso"] = (
        df_final["Centroid_Diag_Coord_Iso"] - df_final["PP_Diag_Coord_Iso"]
    )
    df_final["Centroid_PP_Diag_Diff_Aligned"] = (
        df_final["Centroid_Diag_Coord_Aligned"] - df_final["PP_Diag_Coord_Aligned"]
    )
    df_final["Centroid_PP_Orth_Diff_Iso"] = (
        df_final["Centroid_Orth_Coord_Iso"] - df_final["PP_Orth_Coord_Iso"]
    )
    df_final["Centroid_PP_Orth_Diff_Aligned"] = (
        df_final["Centroid_Orth_Coord_Aligned"] - df_final["PP_Orth_Coord_Aligned"]
    )
    ...
    df_final_L = df_final_L.rename(
        index=str,
        columns={
            "Centroid_PP_Diag_Diff_Iso": "Centroid_PP_Diag_Diff_Iso_L",
    ...
    df_final_R = df_final_R.rename(
        index=str,
        columns={
            "Centroid_PP_Diag_Diff_Iso": "Centroid_PP_Diag_Diff_Iso_R",
    ...
    print(df_final_RM)
    df_final_RM.to_csv(
        os.path.join(DIR_OUT, "inter_tables", "centroid_pp_diag_new_coordinate_RM.csv")
    )
    for label in [0, 1, 2, 3, 4]:
        ...
        df_final_label_RM.to_csv(
            os.path.join(
                DIR_OUT,
                "inter_tables",
                "centroid_pp_diag_new_coordinate_RM_" + str(label) + ".csv",
            )
        )
