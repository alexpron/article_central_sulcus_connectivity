...
if __name__ == '__main__':
    ...
    for name, group in df.groupby('Label'):
        ...
        fused_hemi_level.to_csv(os.path.join(DIR_JAM, 'volume_streamlines_label_level' + str(name) + '.csv'),
                                index=False)

        # transform the data into Repeated Measures (RM) format
        init_variables = ['Mesh_Area', 'Fiedler_Length', 'Roi_Area', 'Max_Geo_Depth', 'PP_CS_Depth',
                          'U_Tract_Volume_vox', 'U_Tract_Volume_mm3']
        ...
        R = R.drop(columns=['Gender', 'Age_in_Yrs', 'Handedness', 'PMAT24_A_CR', 'Dexterity_AgeAdj', 'Strength_AgeAdj',
                            'HandednessQ', 'AgeQ',
                            'PMAT24_A_CR_Q', 'ICV'])
        ...
        for i in range(len(init_variables)):
            final_df[Ass_var[i]] = 2.0 * (final_df[R_var[i]] - final_df[L_var[i]]) / (
