...
if __name__ == "__main__":
    ...
    d = "global_mean"
    path_coords_aligned = os.path.join(
        DIR_CON,
        "data",
        "output",
        "connectivity",
    ...
    for j, side in enumerate(SIDES):
        path_hemi_labels = os.path.join(
            DIR_CON,
            "data",
            "output",
            "group_clustering",
            "dbscan_fixed",
            d + "_" + "all_labels" + "_" + side + ".npy",
        )
    ...
    df = pd.DataFrame(
        {
            ...
            "Pre_Coord_Iso": coords_iso[:, 0],
            "Post_Coord_Iso": coords_iso[:, 1],
            "Pre_Coord_Aligned": coords_aligned[:, 0],
            "Post_Coord_Aligned": coords_aligned[:, 1],
            "Label": labels,
        }
    )
