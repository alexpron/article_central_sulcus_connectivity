...
if __name__ == '__main__':
    ...
    df.to_csv(os.path.join(DIR_JAM, 'inter', 'nb_streamlines_label_level.csv'), index=False)

    for label, subdf in df.groupby('Label'):
        subdf.to_csv(os.path.join(DIR_JAM, 'inter', 'nb_streamlines_label_level_' + str(label) + '.csv'), index=False)
