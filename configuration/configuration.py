...
PEAKS = {subject: os.path.join(DIR_DWI[subject], 'csd', CSD_MODEL, FIT_INSTANCE, 'peaks.nii.gz') for subject in
         SUBJ_LIST}
TRACTS = {(subject, tract_type ): os.path.join(DIR_DWI[subject], 'tractography', subject + 'track.' + tract_type) for subject in SUBJ_LIST for tract_type in ['tck', 'trk']}
...
MESHES = {(subject, side, mesh_type): os.path.join(DIR_MESHES, subject + '_' + side + '_' + mesh_type + '.gii') for
          subject in SUBJ_LIST for side in SIDES.keys() for mesh_type in MESHES_TYPE}
DEPTHS = {(subject, side, 'white'): os.path.join(DIR_MESHES, subject + '_' + side + '_' + 'white_depth.gii') for subject
          in SUBJ_LIST for side in SIDES.keys()}
DPFS = {(subject, side, 'white'): os.path.join(DIR_MESHES, subject + '_' + side + '_' + 'white_dpf.gii') for subject in
        SUBJ_LIST for side in SIDES.keys()}
CURVATURES = {(subject, side, 'white'): os.path.join(DIR_MESHES, subject + '_' + side + '_' + 'white_curvature.gii') for
...
EXTREMITIES = {(subject, side): os.path.join(DIR_LANDMARKS, 'extremities',
                                             subject + '_' + side + '_' + SULCUS + '_' + 'extremities'
                                                                                         '.gii') for
               subject in SUBJ_LIST for side in SIDES.keys()}

GYRAL_CRESTS = {(subject, side, gyrus, status, nature): os.path.join(DIR_LANDMARKS, 'adjacent_gyri',
                                                                     subject + '_' + side + '_' + gyrus + '_' + status +
                                                                     EXTENSIONS[nature]) for subject in SUBJ_LIST for
                side in SIDES.keys() for gyrus in ADJACENT_GYRI[SULCUS] for status in STATUS for nature in EXTENSIONS}
SULCUS_FUNDI = {(subject, side, SULCUS, status, nature): os.path.join(DIR_LANDMARKS, 'fundi',
                                                                      subject + '_' + side + '_' + SULCUS + '_' + status +
                                                                      EXTENSIONS[nature]) for subject in SUBJ_LIST for
                side in SIDES.keys() for gyrus in ADJACENT_GYRI[SULCUS] for status in STATUS for nature in EXTENSIONS}
...
GYRAL_PARAMETRISATIONS = {(subject, side, gyrus, status, nature, param): os.path.join(DIR_LANDMARKS, 'adjacent_gyri',
                                                                                      subject + '_' + side + '_' + gyrus + '_' + status + '_' + param + '_param' +
                                                                                      EXTENSIONS[nature]) for subject in
                          SUBJ_LIST for side in SIDES.keys() for gyrus in ADJACENT_GYRI[SULCUS] for status in STATUS for
                          nature in EXTENSIONS for param in PARAMETRISATIONS}
FUNDI_PARAMETRISATIONS = {(subject, side, SULCUS, status, nature, param): os.path.join(DIR_LANDMARKS, 'fundi',
                                                                                       subject + '_' + side + '_' + gyrus + '_' + status + '_' + param + '_param' +
                                                                                       EXTENSIONS[nature]) for subject
